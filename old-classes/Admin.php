...
class Admin {
    ...
    public function filter_results()
    {
        ...
        $type_of_view = 'ostala';
        $additional_condition = " AND rezervacije.skola_id IS NULL AND rezervacije.razred_id IS NULL";

        if(isset($_GET['type']) && $_GET['type'] == 'skolska') {
            $type_of_view = 'skolska';
        ...
        if ($type_of_view == 'ostala' || $type_of_view == 'skolska') {
            $where_clause .= ($where_clause ? ' AND ' : ' WHERE ') . 'putovanje.type = %s';
            $prepared_values[] = $type_of_view;
        }
    ...
    {
        $where_type = '';
        if($type == 'skolska' || $type == 'ostala') {
            $where_type = " && p.type = '" . $type . "'";
        }
    ...
    public function dc_handle_trip()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($_POST['submit_type']) && $_POST['submit_type'] == 'create_new') {
                ...
                // store razredi
                if(isset($_POST['razredi']) && isset($skola_id)) {
    ...
    function generateUniqueSifra($type = 'skolska') {
        ...
        while (!$done) {
            $unique_sifra = $prefix . mt_rand(100, 9999);
