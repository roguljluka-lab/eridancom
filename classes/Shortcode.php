...
class Shortcode
{
    ...
    public function finish_booking($putovanje, $nova_rezervacija_id, $postoji_duplikat_rezervacija, $dc_first_step_data, $eracuni_poziv_na_broj, $parameters)
    {
        ...
        if($putovanje->type == 'skolska' || $putovanje->type == 'izlet') { // IZLETI
            include_once(DC_REZERVACIJE_PATH . 'views/admin/ugovor-uplatnica/ugovor_pdf.php'); // IZLETI
        } else { // IZLETI
            include_once(DC_REZERVACIJE_PATH . 'views/admin/ugovor-uplatnica/ugovor_pdf_ostala.php'); // IZLETI
        } // IZLETI
        ...
        $is_izlet = ($putovanje->type == 'izlet'); // IZLETI
        if ($is_izlet) { // IZLETI
            $_POST['nacin_placanja'] = 1; // IZLETI
        } // IZLETI
        $nacin_placanja = (int) ($_POST['nacin_placanja'] ?? 1); // IZLETI

        if($is_izlet) { // IZLETI

            // iznos putovanja // IZLETI
            $iznos_za_uplatu = $putovanje->ukupni_iznos; // IZLETI
            $parameters['methodOfPayment'] = 'BankTransfer'; // IZLETI

        } else if($nacin_placanja == 4) { // IZLETI

            // iznos putovanja
            $iznos_za_uplatu = $putovanje->ukupni_iznos;
            $parameters['methodOfPayment'] = 'BankTransfer';

        } else if($nacin_placanja == 3) { // IZLETI
        ...
        } else if($nacin_placanja == 5) { // IZLETI
        ...
        if($postoji_duplikat_rezervacija === false) {
            ...
            $rezervacija_data = array(
                ...
                'informacije' => $dc_first_step_data['informacije'],
                'nacin_placanja' => $nacin_placanja // IZLETI
            );
        ...
        if(/*$_POST['nacin_placanja'] == 1 || */$nacin_placanja == 3 || $nacin_placanja == 5) { // IZLETI
            ...
            $return_url = $_SESSION['dc_return_url'] ?? get_permalink($this->dc_settings->stranica_rezervacije); // IZLETI
            wp_redirect($is_izlet ? $return_url : get_permalink($this->dc_settings->stranica_rezervacije)); // IZLETI
            exit();
    ...
    public function handle_dc_form_submit() {
        if(isset($_POST['new_reservation'])) {
            ...
            $putovanje_id = (int) $_POST['putovanje_id']; // IZLETI
            $query_putovanje = $this->wpdb->prepare("SELECT type FROM {$this->putovanja_table} WHERE id = %d LIMIT 1", $putovanje_id); // IZLETI
            $putovanje = $this->wpdb->get_row($query_putovanje); // IZLETI
            $return_url = $_SESSION['dc_return_url'] ?? get_permalink($this->dc_settings->stranica_rezervacije); // IZLETI

            if ($putovanje && ($putovanje->type == 'skolska' || $putovanje->type == 'izlet')) { // IZLETI
                ...
                if ($postojeci_putnik_id > 0) {
                    ...
                    if ($duplikat_rez > 0) {
                        ...
                        $_SESSION['dc_danger'] = 'Već ste prijavili ovog učenika na odabrano putovanje. Ako mislite da je greška, kontaktirajte agenciju.'; // IZLETI
                        wp_redirect($return_url); // IZLETI
                        exit();
                    }
                }
            }

            if ($putovanje && $putovanje->type == 'izlet' && empty($_POST['razrednik'])) { // IZLETI
                $_POST['razrednik'] = 'RAZREDNIK'; // IZLETI
            } // IZLETI

            // step 2 store to session
            $_SESSION['dc_first_step_data'] = $_POST;
            wp_redirect(($putovanje && $putovanje->type == 'izlet' ? $return_url : get_permalink($this->dc_settings->stranica_rezervacije)) . '/?korak=3'); // IZLETI
            exit();

        } else if(isset($_POST['store_new_reservation'])) {

            $currentDate = current_time('mysql'); // IZLETI
            $dc_first_step_data = $_SESSION['dc_first_step_data']; // IZLETI
            $return_url = $_SESSION['dc_return_url'] ?? get_permalink($this->dc_settings->stranica_rezervacije); // IZLETI
            $postoji_duplikat_rezervacija = false;
            ...
            if($ukupno_mjesta > $zauzeto_mjesta) {
                ...
                if($putovanje->type == 'ostala') { // IZLETI
                    ...
                    $is_izlet = ($putovanje->type == 'izlet'); // IZLETI
                    $nacin_placanja = $is_izlet ? 1 : (int) $_POST['nacin_placanja']; // IZLETI
                    ...
                    if (!empty($result_putnik)) {
                        ...
                        // dodaj novog putnika
                        $putnik_data = array( // IZLETI
                            'ime' => $dc_first_step_data['ime_putnika'], // IZLETI
                            'prezime' => $dc_first_step_data['prezime_putnika'], // IZLETI
                            'adresa' => $dc_first_step_data['adresa_putnika'], // IZLETI
                            'pb' => $dc_first_step_data['postanski_putnika'], // IZLETI
                            'mjesto' => $dc_first_step_data['mjesto_putnika'], // IZLETI
                            'oib_putnika' => $dc_first_step_data['oib_putnika'], // IZLETI
                            'rodendan' => $dc_first_step_data['rodenje_putnika'] ?? null, // IZLETI
                            'spol' => $dc_first_step_data['spol_putnika'] ?? null, // IZLETI
                            'vrsta_isprave' => $dc_first_step_data['vrsta_isprave_putnika'] ?? null, // IZLETI
                            'broj_isprave' => $dc_first_step_data['broj_isprave_putnika'] ?? null, // IZLETI
                            'broj_zdravstvene' => $dc_first_step_data['broj_zdravstvene_putnika'] ?? null, // IZLETI
                            'isprava_vrijedi' => $dc_first_step_data['putna_vrijedi_do_putnika'] ?? null, // IZLETI
                            'kontakt' => $is_izlet ? ($dc_first_step_data['kontakt_ugovaratelja'] ?? '') : $dc_first_step_data['kontakt_putnika'], // IZLETI
                            'created_at' => $currentDate, // IZLETI
                            'tip' => 'putnik' // IZLETI
                        ); // IZLETI
                    ...
                    $all_participants = $this->prepare_participants($dc_first_step_data); // IZLETI
                    $parameters = $is_izlet // IZLETI
                        ? $this->prepare_parameters_izlet($nova_rezervacija_id, $putovanje, $dc_first_step_data, $all_participants) // IZLETI
                        : $this->prepare_parameters($nova_rezervacija_id, $putovanje, $dc_first_step_data, $all_participants); // IZLETI
    ...
    $query  = "SELECT id, broj_putnika, type
               FROM {$this->putovanja_table}
               WHERE id = %d AND status = %d AND putovanje_do >= %s"; // IZLETI
    ...
    if ( $result !== null ) {
        ...
        if ( $slobodno_mjesta > $zauzeto_mjesta ) {
            ...
            wp_redirect( ($result->type == 'izlet' ? $return_url : get_permalink( $this->dc_settings->stranica_rezervacije )) . '/?korak=2' ); // IZLETI
            exit;
    ...
    public function prepare_parameters_izlet($nova_rezervacija_id, $putovanje, $dc_first_step_data, $all_participants) // IZLETI
    { // IZLETI
        // parametri se moraju definirati radi ugovora // IZLETI
        $parameters = array( // IZLETI
            'licence' => $this->dc_settings->licenca, // IZLETI
            'md5pass' => $this->dc_settings->eracuni_password, // IZLETI
            'token' => $this->dc_settings->eracuni_token, // IZLETI
            'eracuni_broj_artikla' => $this->dc_settings->eracuni_broj_artikla, // IZLETI
            'reservation_id' => $nova_rezervacija_id, // IZLETI
            'number' => $putovanje->eracuni_number, // IZLETI
            'naziv' => $putovanje->naziv, // IZLETI
            'sifra' => $putovanje->sifra, // IZLETI
            'ukupni_iznos_putovanja' => $_POST['ukupni_iznos_putovanja'] ?? $putovanje->ukupni_iznos, // IZLETI
            'ime_prezime_putnika' => $dc_first_step_data['ime_putnika'] . ' ' . $dc_first_step_data['prezime_putnika'], // IZLETI
            'ime_ugovaratelja' => $dc_first_step_data['ime_ugovaratelja'], // IZLETI
            'prezime_ugovaratelja' => $dc_first_step_data['prezime_ugovaratelja'], // IZLETI
            'kontakt' => $dc_first_step_data['kontakt_ugovaratelja'], // IZLETI
            'email' => $dc_first_step_data['email_ugovaratelja'], // IZLETI
            'adresa_ugovaratelja' => $dc_first_step_data['adresa_putnika'] ?? '', // IZLETI
            'mjesto_ugovaratelja' => $dc_first_step_data['mjesto_putnika'] ?? '', // IZLETI
            'post_ugovaratelja' => $dc_first_step_data['postanski_putnika'] ?? '', // IZLETI
            'all_participants' => $all_participants ?? null, // IZLETI
            'dc_naziv_skole' => $dc_first_step_data['dc_naziv_skole'] ?? '', // IZLETI
            'dc_razred' => $dc_first_step_data['dc_razred'] ?? '', // IZLETI
            'text_ugovora' => $putovanje->text_ugovora, // IZLETI
            'eracuni_posl_prostor' => $this->dc_settings->eracuni_posl_prostor, // IZLETI
        ); // IZLETI
        // parametri kraj // IZLETI

        return $parameters; // IZLETI
    } // IZLETI
    ...
    public function dc_reset_session()
    {
        ...
        unset($_SESSION['dc_return_url']); // IZLETI
